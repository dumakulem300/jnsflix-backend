...
// Parse Firebase service account JSON safely handling escaped strings
let serviceAccount;
try {
  // Cleans up potential escaped newline issues common in .env strings
  const cleanJson = process.env.FIREBASE_SERVICE_ACCOUNT_JSON.replace(/\\n/g, '\n');
...
/**
 * FIX 1: Webhook Raw Body Middleware Placement
...
app.get('/api/subscription-status', verifyFirebaseToken, async (req, res) => {
  try {
    ...
    if (endDate < now) {
      // FIX 3: Added missing await here to prevent background race conditions
...
/**
 * FIX 2: Correct Webhook Signature Verification for PayMongo
 */
